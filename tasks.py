...
def generate_captions(self, media_path, media_type, style='high_engagement', custom_prompt=None, target_audience=None,
    ...
    """Generate captions using Gemini API with optimized processing for reels"""
    ...
    try:
        # Use faster model for reels to reduce processing time
        if media_type == 'reel':
            model = genai.GenerativeModel('gemini-1.5-flash')  # Already the fastest model
            logger.info("Using optimized processing for reel caption generation")
        else:
            ...
            try:
                ...
                if is_url:
                    try:
                        ...
                        # Optimized download for reels - smaller timeout for faster processing
                        timeout = 15 if media_type == 'reel' else 30
                        response = requests.get(path, stream=True, timeout=timeout)
                ...
                # Read media file based on type - optimized for reels
                if media_type == 'image' or media_type == 'carousel':
                    ...
                    try:
                        ...
                        # For reels, get a frame from the middle for better representation
                        if media_type == 'reel':
                        ...
                        cap.release()
                        logger.info(f"Processed reel frame for caption generation")
                    except Exception as e:
        ...
        if media_type == 'reel':
            base_prompt = f"""
            Create {num_variants} engaging Instagram Reel captions. Be concise and impactful.
            
            Requirements:
            1. Strong hook (first 5 words are crucial)
            2. 1-2 emojis max 
            3. Include: #Reels #Trending
            4. Add 1 engaging question
            5. Keep under 125 characters
            
            Target: {target_audience or 'Food lovers and foodies'}
            Tone: {style.replace('_', ' ').title()}
            """
        elif media_type == 'carousel':
            base_prompt = f"""
            Create {num_variants} Instagram carousel captions for {len(media_inputs)} images.
            
            Requirements:
            1. Hook + "Swipe for more →"
            2. Reference the image journey
            3. Include #carousel hashtags
            4. Clear CTA to save/share
            """
        else:
            base_prompt = f"""
            Create {num_variants} engaging Instagram captions.
            Focus on authenticity and engagement.
            """

        # Simplified style prompts for faster processing
        style_additions = {
            'high_engagement': "Use trending phrases, add question, strong CTA",
            'viral_potential': "Trend-focused, shareable, surprise element", 
            'story_style': "Personal narrative, behind-scenes feel",
            'targeted': f"Target: {target_audience}, Goals: {business_goals}",
            'custom': custom_prompt or ""
        }

        prompt = base_prompt + "\n" + style_additions.get(style, style_additions['high_engagement'])

        # Simplified JSON request for faster response
        prompt += """
        Return only JSON format:
        {"captions": [{"text": "caption", "score": 85}, {"text": "caption", "score": 90}]}
        """

        # Generate with timeout for reels
        try:
            ...
            logger.info(f"Caption generation completed in {generation_time:.2f} seconds")
            ...
            try:
                ...
                if response_text.strip().startswith('{'):
                    ...
                    if json_match:
                        captions_data = json.loads(json_match.group(0))
                    else:
                        raise json.JSONDecodeError("No JSON found", response_text, 0)

                if isinstance(captions_data, dict) and 'captions' in captions_data:
                    # Add reel-specific hashtags if missing
                    if media_type == 'reel':
                        for caption in captions_data['captions']:
                            if '#Reels' not in caption['text']:
                                caption['text'] += ' #Reels #Trending'
                    
                    logger.info(f"Generated {len(captions_data['captions'])} captions for {media_type}")
                    return captions_data

            except json.JSONDecodeError:
                # Fast fallback for reels
                if media_type == 'reel':
                    fallback_captions = [
                        {"text": f"🔥 {response_text[:100]}... #Reels #Trending", "score": 75},
                        {"text": f"✨ Don't miss this! {response_text[:80]}... #FoodReel #Viral", "score": 80}
                    ]
                    logger.warning("Using fallback captions for reel")
                    return {'captions': fallback_captions}
...
@celery_app.task
def fetch_trending_data(webhook_url: str) -> Optional[List[Dict[str, Any]]]:
    """
    Fetches trending data from a given N8n webhook URL.
    """
    logger.info(f"Attempting to fetch trending data from: {webhook_url}")
    try:
        response = requests.get(webhook_url, timeout=30)
        response.raise_for_status() # Raise an HTTPError for bad responses (4xx or 5xx)
        trending_data = response.json()
        logger.info(f"Successfully fetched trending data. Items: {len(trending_data)}")
        return trending_data
    except requests.exceptions.RequestException as e:
        logger.error(f"Error fetching trending data from {webhook_url}: {e}")
        return None
    except json.JSONDecodeError as e:
        logger.error(f"Error decoding JSON from {webhook_url}: {e}. Response content: {response.text[:500]}...")
        return None
    except Exception as e:
        logger.error(f"An unexpected error occurred while fetching trending data from {webhook_url}: {e}")
        return None
...
def verify_facebook_permissions(access_token: str) -> bool:
    ...
    try:
        ...
        page_response = robust_session.get(
...
async def post_media_to_facebook(media_urls: List[str], media_type: str, caption: str) -> Tuple[bool, str]:
    ...
    try:
        if media_type == 'carousel':
            ...
            # First, get the page access token with DNS and network checking
            logger.info("Checking DNS and network connectivity for Facebook...")
            dns_ok, dns_result = check_dns_resolution()
            if not dns_ok:
                error_msg = f"DNS resolution failed for Facebook: {dns_result}"
                logger.error(error_msg)
                return False, error_msg
            
            network_ok, network_result = test_network_connectivity()
            if not network_ok:
                error_msg = f"Network connectivity test failed for Facebook: {network_result}"
                logger.error(error_msg)
                return False, error_msg

            # Get page access token with retries
            logger.info("Getting Facebook page access token with robust session...")
            for attempt in range(3):
                try:
                    page_response = robust_session.get(
                'https://graph.facebook.com/v19.0/me/accounts',
                        params={'access_token': access_token},
                        timeout=(15, 45)  # Increased timeout
                    )

                    if page_response.ok:
                        break
                    else:
                        error_data = page_response.json() if page_response.content else {'error': 'No response'}
                        logger.warning(f"Page access attempt {attempt + 1} failed: {error_data}")
                        if attempt < 2:
                    ...
                    logger.warning(f"Timeout on page access attempt {attempt + 1}")
                    if attempt < 2:
                        time.sleep(10)
                        continue
                    else:
                        return False, "Timeout while getting Facebook page access"
                except requests.exceptions.ConnectionError as e:
                    logger.warning(f"Connection error on page access attempt {attempt + 1}: {str(e)}")
                    if attempt < 2:
                        time.sleep(15)
                        continue
                    else:
                        return False, f"Connection error while getting Facebook page access: {str(e)}"
            ...
            if media_type == 'reel':
                try:
                    ...
                    response = robust_session.get(media_urls[0], stream=True, timeout=(30, 120))
                    response.raise_for_status()
                    ...
                    init_response = robust_session.post(
                    ...
                    with open(temp_file_path, 'rb') as video_file:
                        ...
                        while True:
                            ...
                            chunk_response = robust_session.post(
                                f'https://graph.facebook.com/v19.0/{Config.FACEBOOK_PAGE_ID}/videos',
                                data=chunk_data,
                                files=files
                            )
                    ...
                    finish_response = robust_session.post(
                        f'https://graph.facebook.com/v19.0/{Config.FACEBOOK_PAGE_ID}/videos',
                        data=finish_data
                    )
                ...
                # For photos, use simple upload with robust session
                post_data = {
                    'url': media_urls[0],
                    'caption': caption,
                    'access_token': page_token
                ...
                logger.info("Posting photo to Facebook with robust session...")
                for attempt in range(3):
                    try:
                    ...
                    f'https://graph.facebook.com/v19.0/{Config.FACEBOOK_PAGE_ID}/photos',
                        ...
                        if response.ok:
                            logger.info("Successfully posted photo to Facebook")
                            return True, response.json().get('id')
                        else:
                            error_data = response.json() if response.content else {'error': 'No response'}
                            logger.warning(f"Photo post attempt {attempt + 1} failed: {error_data}")
                            if attempt < 2:
                                time.sleep(5 * (attempt + 1))
                                continue
                            else:
                        ...
                        logger.warning(f"Timeout on photo post attempt {attempt + 1}")
                        if attempt < 2:
                            time.sleep(10)
                            continue
                        else:
                            return False, "Timeout while posting photo to Facebook"
                    except requests.exceptions.ConnectionError as e:
                        logger.warning(f"Connection error on photo post attempt {attempt + 1}: {str(e)}")
                        if attempt < 2:
                            time.sleep(15)
                            continue
                        else:
                            return False, f"Connection error while posting photo to Facebook: {str(e)}"

    except Exception as e:
        logger.error(f"Critical error posting to Facebook: {str(e)}")
...
async def post_carousel_to_facebook(media_urls: List[str], caption: str) -> Tuple[bool, str]:
    """Post carousel media to Facebook as individual photos with enhanced error handling"""
    try:
        logger.info("Starting Facebook carousel post with enhanced error handling...")
        
        # Check DNS and network connectivity first
        dns_ok, dns_result = check_dns_resolution()
        if not dns_ok:
            error_msg = f"DNS resolution failed for Facebook carousel: {dns_result}"
            logger.error(error_msg)
            return False, error_msg
        
        network_ok, network_result = test_network_connectivity()
        if not network_ok:
            error_msg = f"Network connectivity test failed for Facebook carousel: {network_result}"
            logger.error(error_msg)
            return False, error_msg
        ...
        # Get page access token with retries using robust session
        logger.info("Getting Facebook page access token for carousel...")
        for attempt in range(3):
            try:
                page_response = robust_session.get(
            'https://graph.facebook.com/v19.0/me/accounts',
                ...
                if attempt < 2:
                    ...
                    return False, "Timeout while getting Facebook page access for carousel"
            except requests.exceptions.ConnectionError as e:
                logger.warning(f"Connection error on page access attempt {attempt + 1}: {str(e)}")
                if attempt < 2:
                    time.sleep(15)
                    ...
                    return False, f"Connection error while getting Facebook page access for carousel: {str(e)}"
        ...
        # Post first image with caption using robust session
        first_post_data = {
            'url': media_urls[0],
            'caption': caption,
            'access_token': page_token
        ...
        logger.info("Posting first image of carousel to Facebook...")
        for attempt in range(3):
            try:
                response = robust_session.post(
            f'https://graph.facebook.com/v19.0/{Config.FACEBOOK_PAGE_ID}/photos',
                ...
                if response.ok:
                    ...
                    error_data = response.json() if response.content else {'error': 'No response'}
                    logger.warning(f"First image post attempt {attempt + 1} failed: {error_data}")
                    if attempt < 2:
                        time.sleep(5 * (attempt + 1))
                        continue
                    else:
                ...
                logger.warning(f"Timeout on first image post attempt {attempt + 1}")
                if attempt < 2:
                    time.sleep(10)
                    continue
                else:
                    return False, "Timeout while posting first image to Facebook"
            except requests.exceptions.ConnectionError as e:
                logger.warning(f"Connection error on first image post attempt {attempt + 1}: {str(e)}")
                if attempt < 2:
                    time.sleep(15)
                    continue
                else:
                    return False, f"Connection error while posting first image to Facebook: {str(e)}"
        
        first_post_id = response.json().get('id')
        logger.info(f"Successfully posted first image to Facebook: {first_post_id}")
        
        # Post remaining images without caption using robust session
        for i, media_url in enumerate(media_urls[1:], 2):
            ...
            logger.info(f"Posting additional image {i} of {len(media_urls)} to Facebook...")
            for attempt in range(3):
                try:
                    response = robust_session.post(
                f'https://graph.facebook.com/v19.0/{Config.FACEBOOK_PAGE_ID}/photos',
                    ...
                    if response.ok:
                        logger.info(f"Successfully posted image {i} to Facebook")
                        break
                    else:
                        error_data = response.json() if response.content else {'error': 'No response'}
                        logger.warning(f"Image {i} post attempt {attempt + 1} failed: {error_data}")
                        if attempt < 2:
                            time.sleep(3 * (attempt + 1))
                            continue
                        else:
                            logger.warning(f"Failed to post additional image {i} in carousel: {error_data}")
                # Continue with remaining images even if one fails
                            break
                            
                except requests.exceptions.Timeout:
                    logger.warning(f"Timeout on image {i} post attempt {attempt + 1}")
                    if attempt < 2:
                        time.sleep(8)
                        continue
                    else:
                        logger.warning(f"Timeout while posting image {i} to Facebook - continuing with next image")
                        break
                except requests.exceptions.ConnectionError as e:
                    logger.warning(f"Connection error on image {i} post attempt {attempt + 1}: {str(e)}")
                    if attempt < 2:
                        time.sleep(10)
                        continue
                    else:
                        logger.warning(f"Connection error while posting image {i} to Facebook - continuing with next image")
                        break
        
        logger.info(f"Completed Facebook carousel post with first post ID: {first_post_id}")
...
def check_dns_resolution(hostname="graph.facebook.com"):
    """Check if DNS can resolve Facebook's Graph API with fallback DNS servers"""
    import dns.resolver
    
    # List of public DNS servers to try
    dns_servers = [
        None,  # System default
        '8.8.8.8',  # Google DNS
    ...
    for dns_server in dns_servers:
        try:
            if dns_server:
                ...
                # Try with custom resolver
                result = resolver.resolve(hostname, 'A')
                ip = str(result[0])
                logger.info(f"DNS resolution successful using {dns_server}: {hostname} -> {ip}")
                return True, ip
            else:
                # Try system default DNS
                logger.info("Trying system default DNS")
                result = socket.gethostbyname(hostname)
                logger.info(f"DNS resolution successful with system DNS: {hostname} -> {result}")
                return True, result
                
        except (socket.gaierror, dns.resolver.NXDOMAIN, dns.resolver.NoAnswer, dns.resolver.Timeout) as e:
            logger.warning(f"DNS resolution failed with {'system DNS' if not dns_server else dns_server}: {e}")
            continue
        except Exception as e:
            logger.warning(f"Unexpected DNS error with {'system DNS' if not dns_server else dns_server}: {e}")
            continue
    
    logger.error(f"DNS resolution failed with all DNS servers for {hostname}")
    return False, "All DNS servers failed"

# Enhanced robust session with DNS checking
def create_robust_session():
    ...
    dns_ok, dns_result = check_dns_resolution()
    if not dns_ok:
        logger.warning(f"DNS resolution issue detected: {dns_result}")
        # Continue anyway, let the requests library handle it
    
    # Define retry strategy with more aggressive settings
    retry_strategy = Retry(
        total=5,  # Increased from 3 to 5 retries
        backoff_factor=2,  # Increased backoff
        status_forcelist=[429, 500, 502, 503, 504],
        raise_on_status=False,
        # Add specific retry for connection errors
    ...
    # Set default headers to help with connectivity
    session.headers.update({
        'User-Agent': 'Social-Media-Automation/1.0',
...
def test_network_connectivity():
    """Test basic network connectivity to Facebook API"""
    try:
        logger.info("Testing network connectivity to Facebook Graph API...")
        
        # Use global session if available, otherwise create a basic one
        try:
        ...
        return False, "Network timeout - please check your internet connection"
    except requests.exceptions.ConnectionError as e:
...
def get_instagram_account_id() -> Optional[str]:
    """Get Instagram account ID from Facebook Page ID with enhanced error handling"""
    try:
        logger.info("Getting Instagram account ID...")
        
        # First check DNS resolution and network connectivity
        dns_ok, dns_result = check_dns_resolution()
        if not dns_ok:
        ...
        network_ok, network_result = test_network_connectivity()
        if not network_ok:
            logger.error(f"Network connectivity test failed: {network_result}")
            return None
        
        # Try multiple attempts with different strategies
        for attempt in range(3):
            try:
                logger.info(f"Attempt {attempt + 1} to get Instagram account ID")
                ...
                if response.ok:
                    ...
                    if instagram_id:
                        logger.info(f"Successfully retrieved Instagram account ID: {instagram_id}")
                        return instagram_id
                    else:
                        logger.error("No Instagram business account found in response")
                        return None
                else:
                    error_data = response.json() if response.content else {'error': 'No response content'}
                    logger.warning(f"API error (attempt {attempt + 1}): {error_data}")
                    
                    if attempt < 2:  # Not the last attempt
                        time.sleep(5 * (attempt + 1))  # Progressive delay
                        continue
                    else:
                        logger.error(f"Failed after {attempt + 1} attempts: {error_data}")
                        return None
                ...
                if attempt < 2:
                    ...
                    logger.error("Timeout on all attempts")
                    return None
            except requests.exceptions.ConnectionError as e:
                logger.warning(f"Connection error on attempt {attempt + 1}: {str(e)}")
                if attempt < 2:
                    time.sleep(15)  # Longer delay for connection errors
                    continue
                else:
                    logger.error(f"Connection failed on all attempts: {str(e)}")
                    return None
            except Exception as e:
                ...
                if attempt < 2:
                    ...
                    logger.error(f"Failed on all attempts: {str(e)}")
                    return None

    except Exception as e:
        logger.error(f"Critical error getting Instagram account ID: {str(e)}")
        return None

async def post_photo_to_instagram(media_url: str, caption: str) -> Tuple[bool, str]:
    """Post a single photo to Instagram with improved error handling"""
    try:
        # Get Instagram account ID with retries
        instagram_account_id = get_instagram_account_id()
        if not instagram_account_id:
            return False, "Failed to get Instagram account ID"

        logger.info(f"Creating media container for photo: {media_url}")

        # Create container with timeout
        container_response = robust_session.post(
            ...
            timeout=(10, 60)  # Longer timeout for media upload
        )

        if not container_response.ok:
            error_data = container_response.json() if container_response.content else {'error': 'No response content'}
            error_msg = error_data.get('error', {}).get('message', 'Unknown error')
            logger.error(f"Failed to create media container: {error_msg}")
            return False, f"Failed to create media container: {error_msg}"
        ...
        # Publish the container with timeout
        publish_response = robust_session.post(
        ...
        if not publish_response.ok:
            error_data = publish_response.json() if publish_response.content else {'error': 'No response content'}
            error_msg = error_data.get('error', {}).get('message', 'Unknown error')
            logger.error(f"Failed to publish media: {error_msg}")
            return False, f"Failed to publish media: {error_msg}"

        media_id = publish_response.json().get('id')
        logger.info(f"Successfully published photo: {media_id}")
        return True, media_id

    except requests.exceptions.Timeout:
        error_msg = "Timeout while posting photo to Instagram"
        logger.error(error_msg)
        return False, error_msg
    except requests.exceptions.ConnectionError as e:
        error_msg = f"Connection error while posting photo to Instagram: {str(e)}"
        logger.error(error_msg)
        return False, error_msg
    except Exception as e:
        error_msg = f"Error posting photo to Instagram: {str(e)}"
        logger.error(error_msg)
        return False, error_msg

async def post_carousel_to_instagram(media_urls: List[str], caption: str) -> Tuple[bool, str]:
    """Post carousel media to Instagram with improved error handling"""
    try:
        instagram_account_id = get_instagram_account_id()
        if not instagram_account_id:
            return False, "Failed to get Instagram account ID"

        logger.info(f"Creating carousel with {len(media_urls)} media items")

        # Create containers for each media
        children = []
        for i, media_url in enumerate(media_urls):
            try:
                logger.info(f"Creating container for media item {i+1}: {media_url}")
                ...
                if not container_response.ok:
                    error_data = container_response.json() if container_response.content else {'error': 'No response content'}
                    error_msg = error_data.get('error', {}).get('message', 'Unknown error')
                    logger.error(f"Failed to create media container for item {i+1}: {error_msg}")
                    return False, f"Failed to create media container for item {i+1}: {error_msg}"

                container_id = container_response.json().get('id')
                children.append(container_id)
                logger.info(f"Created container for item {i+1}: {container_id}")
                
                # Small delay between requests
                time.sleep(1)
                
            except requests.exceptions.Timeout:
                error_msg = f"Timeout creating carousel item {i+1}"
                logger.error(error_msg)
                return False, error_msg
            except Exception as e:
                error_msg = f"Error creating carousel item {i+1}: {str(e)}"
                logger.error(error_msg)
                return False, error_msg

        logger.info(f"Created {len(children)} containers, creating carousel container")

        # Create carousel container
        carousel_response = robust_session.post(
            f'https://graph.facebook.com/v19.0/{instagram_account_id}/media',
            params={
                'access_token': Config.INSTAGRAM_ACCESS_TOKEN,
                'media_type': 'CAROUSEL',
        ...
        if not carousel_response.ok:
            error_data = carousel_response.json() if carousel_response.content else {'error': 'No response content'}
            error_msg = error_data.get('error', {}).get('message', 'Unknown error')
            logger.error(f"Failed to create carousel container: {error_msg}")
            return False, f"Failed to create carousel container: {error_msg}"
        ...
        if not response.ok:
            error_data = response.json() if response.content else {'error': 'No response content'}
            error_msg = error_data.get('error', {}).get('message', 'Unknown error')
            logger.error(f"Failed to publish carousel: {error_msg}")
            return False, f"Failed to publish carousel: {error_msg}"
        ...
        error_msg = "Timeout while posting carousel to Instagram"
        logger.error(error_msg)
        return False, error_msg
    except requests.exceptions.ConnectionError as e:
        error_msg = f"Connection error while posting carousel to Instagram: {str(e)}"
        logger.error(error_msg)
        return False, error_msg
    except Exception as e:
        error_msg = f"Error posting carousel to Instagram: {str(e)}"
        logger.error(error_msg)
        return False, error_msg

async def post_reel_to_instagram(media_url: str, caption: str) -> Tuple[bool, str]:
    """Post a reel to Instagram with improved error handling"""
    try:
        instagram_account_id = get_instagram_account_id()
        if not instagram_account_id:
            return False, "Failed to get Instagram account ID"

        logger.info(f"Creating reel container for: {media_url}")

        # Create container for reel with timeout
        container_response = robust_session.post(
            f'https://graph.facebook.com/v19.0/{instagram_account_id}/media',
            params={
                'access_token': Config.INSTAGRAM_ACCESS_TOKEN,
            ...
            timeout=(15, 120)  # Longer timeout for video upload
        )
        ...
        if not container_response.ok:
            ...
            return False, f"Failed to create reel container: {error_msg}"

        container_id = container_response.json().get('id')
        logger.info(f"Created reel container: {container_id}")

        # Check status and wait for video processing with improved error handling
        max_attempts = 10
        attempt = 0
        while attempt < max_attempts:
            try:
                logger.info(f"Checking reel status (attempt {attempt + 1})")
                
                status_response = robust_session.get(
                ...
                if not status_response.ok:
                    error_data = status_response.json() if status_response.content else {'error': 'No response content'}
                    error_msg = error_data.get('error', {}).get('message', 'Unknown error')
                    logger.warning(f"Failed to check reel status (attempt {attempt + 1}): {error_msg}")
                    await asyncio.sleep(5)
                    attempt += 1
                    continue

                status = status_response.json().get('status_code')
                logger.info(f"Reel status: {status}")
                ...
                logger.warning(f"Timeout checking reel status (attempt {attempt + 1})")
                await asyncio.sleep(5)
                attempt += 1
                continue
            except Exception as e:
                logger.error(f"Error checking reel status (attempt {attempt + 1}): {e}")
                await asyncio.sleep(5)
                attempt += 1
                continue
        ...
        logger.info("Reel processing completed, publishing...")

        # Publish the reel with timeout
        response = robust_session.post(
            f'https://graph.facebook.com/v19.0/{instagram_account_id}/media_publish',
            params={
                'access_token': Config.INSTAGRAM_ACCESS_TOKEN,
        ...
        error_msg = "Timeout while posting reel to Instagram"
        logger.error(error_msg)
        return False, error_msg
    except requests.exceptions.ConnectionError as e:
        error_msg = f"Connection error while posting reel to Instagram: {str(e)}"
        logger.error(error_msg)
        return False, error_msg
    except Exception as e:
        error_msg = f"Error posting reel to Instagram: {str(e)}"
        logger.error(error_msg)
        return False, error_msg
