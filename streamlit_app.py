...
if st.session_state["page"] == "trending":
    ...
    topics = [
        "Health", "Fitness", "Food", "Catering", "Gaming",
        "Technology", "Fashion", "Travel", "Sports", "Finance",
        "Education", "Movies", "Music", "Art", "Science"
    ]
    # Show topics as buttons
    for topic in topics:
        if st.button(topic):
            # Send topic to n8n webhook
            webhook_url = "https://3def-49-204-118-224.ngrok-free.app/webhook-test/trending_content"
            response = requests.post(webhook_url, json={"topic": topic})
            if response.status_code == 200:
                data = response.json()
                st.image(data["image_url"])
                st.write(data.get("post_text", ""))
            else:
                st.error("Failed to fetch trending content.")
# Platform selection
